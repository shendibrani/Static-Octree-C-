...
void GameObject::setCollider(Collider* pCollider)
{
	_collider = pCollider;
	_collider->_owner = this;
	glm::mat4 thisTrans = glm::scale (pCollider->_owner->getTransform(), _collider->_halfSize);
	pCollider->_owner->setTransform(thisTrans);
}
